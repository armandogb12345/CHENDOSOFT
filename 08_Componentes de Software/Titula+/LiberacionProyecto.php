...
<body style="background: url('images/milestones_background.jpg');">
        <header class= "header">
                <nav>
                <a href="#"><label>Liberacion del proyecto de: <?php echo $username; ?> :</label> </a>
        ...
        $consulta="SELECT documento.Titulo, docente.NombreDocente, estudiante.NombreEstudiante, expediente.Nombre, jefedepartamento.NombreDep , documento.Aprobado
        ...
        <table class="table table-striped table-dark">
            <thead >
                <tr>
                <th scope="col">Titulo del documento</th>
                <th scope="col">Nombre del asesor/revisor</th>
            ...
            <?php
                if(mysqli_num_rows($resultado)>0){
                    while($r = mysqli_fetch_array($resultado)){
                        echo '<tr>';
                        echo '<td>'.$r['Titulo'].'</td>';
                        echo '<td>'.$r['NombreDocente'].'</td>';
                        echo '<td>'.$r['NombreEstudiante'].'</td>';
                        echo '<td>'.$r['Nombre'].'</td>';
                        echo '<td>'.$r['NombreDep'].'</td>';
                    }
                }
            ?>
            <label><input type="radio" name="validado" value="Validado"required>Validar</label><br>
            <label><input type="radio" name="validado" value="No validado">No validar</label>
            </tbody>
        </table>
        <?php echo '<a class="btn btn-primary" href="principal.php?id='.$id_user.'&type='.$type.'"> Aceptar</a>';
        	   if(isset($_POST[$type])){
        	   		if(isset($_POST['validado'] && $_POST['validado'] == 'Validado')){
            			$consultaValidacion = "update documento set Validado='Si'";
            			$resultado = mysqli_query($conexion, $consultaValidacion);
            		}else {
            			$consultaValidacion = "update documento set Validado='No'";
            			$resultado = mysqli_query($conexion, $consultaValidacion);
            		}
        	   }
        ?>
